...
def set_calc(atoms, mlp, device, return_calc=True,logger=logger):
    ...
    return (atoms, calculator) if return_calc else atoms

def run_mlp(system, atoms, mlp, device, isif=3, logger=logger):
    logger.log("DEBUG", "run_mlp FUNC")
    atoms, calc = set_calc(atoms, mlp, device)
    calculation_type='relaxation'
    if isif == 2:
        atoms = UnitCellFilter(atoms, mask = [False]*6, constant_volume=True)
        calculation_type='static calculation'
    
    optimizer = FIRE(atoms, logfile=os.path.join(os.environ['LOG'],'mlp',f'{system.lower()}.{mlp}.ase'), trajectory=os.path.join(os.environ['TRAJ'],f'{system}.{mlp}.traj'), append_trajectory=True)
    ...
    if return_results:
        return pe, vol, force, stress, calc
    return
